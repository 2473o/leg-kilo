...
void Kinematics::processing(const go2_driver::msg::LegSensor& high_state, common::KinImuMeas& kin_imu_meas) {
    ...
    */
    ...
    for (int i = 0; i < 3; ++i) {
        foot_angle[0][i] = high_state.q[3 + i];
        foot_angle_vel[0][i] = high_state.dq[3 + i];
        foot_angle[1][i] = high_state.q[0 + i];
        foot_angle_vel[1][i] = high_state.dq[0 + i];
        foot_angle[2][i] = high_state.q[9 + i];
        foot_angle_vel[2][i] = high_state.dq[9 + i];
        foot_angle[3][i] = high_state.q[6 + i];
        foot_angle_vel[3][i] = high_state.dq[6 + i];
    }
