    ...
    for (size_t i = 0; i < 8; i++) {
        if (leaves_[i] != nullptr) {
            if (leaves_[i]->temp_points_.size() > static_cast<size_t>(leaves_[i]->points_size_threshold_)) {
                ...
                if (leaves_[i]->plane_ptr_->is_plane_ == true) {
...
void VoxelMapManager::pubVoxelMap(rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr voxel_map_pub) {
    ...
    voxel_map_pub->publish(voxel_plane);
    loop.sleep();
}
